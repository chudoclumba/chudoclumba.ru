...
//	if (!($ord>0) || !is_numeric ($ord)) return "PostOk\n";
 	...
 	if (!(count($rows)>0)) return "Error Заказ не найден\n";
 //	$rows = $db->get_rows("SELECT * FROM ".TABLE_PRODUCTS." where  id=".$ord);
 	$body="<p>Здравствуйте, {$fio}!</p><p>В интернет-магазине «Чудо-клумба» Вами был размещен заказ № {$ord} на Весну 2016 года.</p>
 	<p>Ваш заказ собран в следующем составе:</p>";
 	ob_start();
 ?>
...
<?
	...
	$keylink1=SITE_URL."service/receive/{$ord}/".md5("decline".$ord).'?utm_source=email_transaction&utm_medium=email&utm_campaign=orderdecline';
 	$body.="<p>Если Вы хотите внести изменения в заказ, произвести замену или добавить растения, оформите, пожалуйста, дозаказ. Все растения, представленные в данный момент на сайте, находятся на складе или их поступление ожидается в ближайшее время.</p>
 	<p>Мы отправим новый заказ вместе с основным в одной посылке, после получения Вашего подтверждения.</p>
 	<p>Следите за нашими новостями, каталоги будут постоянно обновляться!</p>
 	<p>После оформления дозаказа или для доставки заказа в текущем составе по адресу: {$adress}, нажмите на кнопку:</p>
	<a href=\"{$keylink}\"><img src=\"".SITE_URL."templates/101/images/mail_key_ok.jpg\" alt=\"Подтвердить заказ!\" width=\"213\" height=\"41\" /></a><br><br>
<p>Если Ваш заказ не актуален, нажмите, пожалуйста, на кнопку:</p>
<a href=\"{$keylink1}\"><img src=\"".SITE_URL."templates/101/images/mail_key_dec.jpg\" alt=\"Отменить заказ!\" width=\"213\" height=\"41\" /></a><br><br><p>Обращаем Ваше внимание, что, если в течение двух рабочих дней ответ от Вас не будет получен, Ваш заказ будет отменен.</p>";
//$mail='seregin_alex@mail.ru';
	$res=sendmail_a($mail,'Ваш заказ.',$body,'?utm_source=email_transaction&utm_medium=email&utm_campaign=orderconfirm');
...
<form name="ddd" action="http://chudo-clumba.ru" method="post">
