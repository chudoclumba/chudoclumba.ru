...
$footer_view = 111;
if(!file_exists("includes/site.class.php")){
...
if (file_exists(ROOT_DIR."login.php"))	require(ROOT_DIR."login.php");
if (isset($sets['yak']) && $sets['yak']==1) {
	require_once "includes/kassayandex/Settings.php";
	require_once "includes/kassayandex/YaMoneyCommonHttpProtocol.php";
}
...
if ($_SERVER['REQUEST_URI'] != '/') {
	$flag=true;
		...
		if ($sets['cpu']==1 && $fl) {  //       ,       
			$rvl=$site->db->get(TABLE_SITEMENU,array('vlink'=>trim($url_path, ' /')));
			if (count($rvl)>0) {
				$_GET['module']='site';
				$_GET['id']=$rvl[0]['id'];
				$flag=false; //    
...
if (isset($_GET['q']) || (!isset($_GET['module']) && !empty($_GET)))  $site->_404();
...
if($_SERVER['REQUEST_URI'] == '/'.DEF_MODULE.'/'.DEF_ID || $_SERVER['REQUEST_URI'] == '/i.php') $site->_404(); // || strpos($_SERVER['REQUEST_URI'], '?') !== false
//=================
