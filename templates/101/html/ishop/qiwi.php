...
<div style="margin:0 auto; padding:5px; width:450px; border:1px solid #ddd; background:#fff; border-radius: 7px; -webkit-border-radius: 7px; -moz-border-radius: 7px; font:normal 14px/14px Geneva,Verdana,Arial,Helvetica,Tahoma,sans-serif;">
	<form action="http://w.qiwi.ru/setInetBill.do" method="get" accept-charset="windows-1251" onSubmit="return checkSubmit();">
		...
		<input type="hidden" name="from" value="9183"/>
		<input type="hidden" name="lifetime" value="1000.0"/>
    	...
    	<!--<input type="hidden" name="txn_id" value="<?=$last_id?>"/> -->
		...
		<table style="border-collapse:collapse;">
			...
			<tr>
				...
				<td style="padding:10px">
					<input type="text" name="amount_rub" value="<?=floor($summacn)?>" maxlength="5" style="width:50px; text-align:right;  border: 1px inset #555;" /> руб. 
					<input type="text" name="amount_kop" value="<?=round(($summacn-floor($summacn))*100)?>" maxlength="2" size="2" style="text-align:right; border: 1px inset #555;"/> коп.
				</td>
			</tr>
			<tr style="background:#f1f5fa;">
				<td style="color:#a3b52d; padding:10px 0px; width:45%; text-align:center;">Комментарий</td>
				<td style="padding:10px"><textarea rows="2" cols="30" name="com"  readonly="readonly" style="width:200px; border: 1px inset #555;">Счет к заказу № <?=$last_id?></textarea></td>
			</tr>
		</table>
