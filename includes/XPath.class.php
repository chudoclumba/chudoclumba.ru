...
//echo "close: $closePos\nopen: $openPos\n\n";
  ...
  function _closeDebugFunction($aStartTime, $returnValue = "") {
    ...
    if (isSet($returnValue)) {
      if (is_array($returnValue))
        ...
        echo "Return Value: \"".htmlspecialchars($returnValue,ENT_COMPAT | ENT_XHTML,'cp1251')."\"\n";
    }
  ...
  function _treeDump($node, $indent = '') {
    ...
    if (empty($indent)) {
      return "<pre>" . htmlspecialchars($out,ENT_COMPAT | ENT_XHTML,'cp1251') . "</pre>";
    }
    return $out;
  }
...
//        $errStr = 'This xml document (string) was empty';
  ...
  function _handleStartElement($parser, $nodeName, $attributes) {
    ...
    if ($this->bDebugXmlParse) {
      echo "<blockquote>" . htmlspecialchars("Start node: <".$nodeName . ">",ENT_COMPAT | ENT_XHTML,'cp1251')."<br>";
      echo "Appended to stack entry: $this->parseStackIndex<br>\n";
      echo "Text part before element is: ".htmlspecialchars($this->parsedTextLocation,ENT_COMPAT | ENT_XHTML,'cp1251');
      /*
      echo "<pre>";
      $dataPartsCount = count($this->nodeStack[$this->parseStackIndex]['textParts']);
      for ($i = 0; $i < $dataPartsCount; $i++) {
        echo "$i:". htmlspecialchars($this->nodeStack[$this->parseStackIndex]['textParts'][$i],ENT_COMPAT | ENT_XHTML,'cp1251')."\n";
      }
      echo "</pre>";
      */
    }
  ...
  function _handleEndElement($parser, $name) {
    ...
    if ($this->bDebugXmlParse) {
      echo "Text part after element is: ".htmlspecialchars($this->parsedTextLocation,ENT_COMPAT | ENT_XHTML,'cp1251')."<br>\n";
      echo htmlspecialchars("Parent:<{$this->parseStackIndex}>, End-node:</$name> '".$this->parsedTextLocation,ENT_COMPAT | ENT_XHTML,'cp1251') . "'<br>Text nodes:<pre>\n";
      $dataPartsCount = count($this->nodeStack[$this->parseStackIndex]['textParts']);
      for ($i = 0; $i < $dataPartsCount; $i++) {
        echo "$i:". htmlspecialchars($this->nodeStack[$this->parseStackIndex]['textParts'][$i],ENT_COMPAT | ENT_XHTML,'cp1251')."\n";
      }
  ...
  function _handleCharacterData($parser, $text) {
    ...
    if ($this->bDebugXmlParse) echo "Handling character data: '".htmlspecialchars($text,ENT_COMPAT | ENT_XHTML,'cp1251')."'<br>";
    if ($this->parseSkipWhiteCache AND !empty($text) AND !$this->parsInCData) {
      ...
      if ($this->bDebugXmlParse) echo "'Skip white space' is ON. reduced to : '" .htmlspecialchars($text,ENT_COMPAT | ENT_XHTML,'cp1251') . "'<br>";
    }
  ...
  function _handleDefaultData($parser, $text) {
    do { // try-block
      ...
      if ($this->bDebugXmlParse) echo "Default handler data: ".htmlspecialchars($text,ENT_COMPAT | ENT_XHTML,'cp1251')."<br>";    
      break; // try-block
    } while (FALSE); // END try-block
  }
