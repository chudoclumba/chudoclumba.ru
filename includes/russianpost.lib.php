...
class RussianPostAPI {
  ...
  public function getOperationHistory($trackingNumber) {
    ...
    if (empty($trackingNumber)) {
      throw new RussianPostArgumentException('Не введен трек номер!');
    }
    elseif (!preg_match('/^[0-9]{14}|[A-Z]{2}[0-9]{9}[A-Z]{2}$/', $trackingNumber)) {
  ...
  protected function parseResponse($raw) {
    ...
    foreach($records as $rec) {
      ...
      $outRecord->declaredValue            = round(floatval($rec->FinanceParameters->Value) / 1, 2);
      $outRecord->collectOnDeliveryPrice   = round(floatval($rec->FinanceParameters->Payment) / 1, 2);
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:data="http://russianpost.org/operationhistory/data">
 <soapenv:Header>
  <data:AuthorizationHeader soapenv:mustUnderstand="1">
   <data:login>SERmitMih</data:login>
   <data:password>SERkhTPHz</data:password>
