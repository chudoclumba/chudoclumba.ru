...
{
	$db->query("UPDATE ".TABLE_SITEMENU." SET title = '".$value."' where `id` = '".$_GET['id']."'");
...
{
	$db->query("UPDATE ".TABLE_SITEMENU." SET position = '".$_GET['value']."' where `id` = '".$_GET['id']."'");
	$buff = "1";
}
elseif($_GET['act'] == 'set_params')
{
	...
	if($_GET['param_name'] == 'mod_change_price')
	{
		if($param['value'] == 1)
		{
			...
			$db->query("ALTER TABLE ".TABLE_PRODUCTS." CHANGE `tsena` `tsena` TEXT NULL DEFAULT NULL");
		}
		else
		{
			$db->update(TABLE_PRD_P, array('param_descr' => 'tsena'), array('param_type' => 'decimal(15,2)'));
			$db->query("ALTER TABLE ".TABLE_PRODUCTS." CHANGE `tsena` `tsena` DECIMAL( 15, 2 ) NULL DEFAULT NULL");
		}
	}

	$db->update(TABLE_SETTINGS, array('id' => $param['id']), array('value' => $param['value']));
	$buff = $param['value'];
}
elseif($_GET['act'] == 'del_comments')
{
	$db->query("DELETE FROM ".TABLE_COMMENTS." WHERE id=".$_GET['id']." && module = 1");
}
elseif ($_GET['act'] == 'part_on')
{
	$row = $db->fetch_array($db->query("SELECT enabled FROM ".TABLE_SITEMENU." where `id` = '".$_GET['id']."'"));

	$vl = ($row['enabled'] == 1) ? 0 : 1;

	$db->query("UPDATE ".TABLE_SITEMENU." SET enabled = '".$vl."' where `id` = '".$_GET['id']."'");
...
{
	$row = $db->fetch_array($db->query("SELECT visible FROM ".TABLE_SITEMENU." where `id` = '".$_GET['id']."'"));
	$param = ($row['visible']=='0')?'1':'0';
	$db->query("UPDATE ".TABLE_SITEMENU." SET visible = '".$param."' where `id` = '".$_GET['id']."'");
